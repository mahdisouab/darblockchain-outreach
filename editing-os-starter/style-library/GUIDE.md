...
├── DESIGN.md         ← the design spec: palette, typography, motion rules, "what not to do" 
...
Committed: `style.json`, `DESIGN.md`, `tokens.css`, the card `.html` files, the **reference images** (the look's source of truth), and the **poster PNGs** (so the catalog is browsable on GitHub). Gitignored: the heavier preview **MP4** loops (regenerable from the cards).
...
- Local `style-templates/` looks are reference only. Borrow ideas, do not port wholesale.

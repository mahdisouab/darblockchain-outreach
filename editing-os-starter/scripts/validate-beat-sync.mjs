#!/usr/bin/env node
// Gate 0 — transcript-sync validator for session-99-percent.
...
// enforces 0.3s <= (wordStart - beatStart) <= 1.5s.
//
// Usage:
...
function parseIndex(html) {
  ...
  const beatRe =
    /<div\s+id="(b[0-9a-z]+)"[^>]*?data-composition-src="([^"]+)"[^>]*?data-start="([^"]+)"[^>]*?data-duration="([^"]+)"[^>]*?>\s*<\/div>/gi;
  const beats = [];
  let m;
...
function resolveStarts(beats) {
  ...
  function resolveOne(id, stack = new Set()) {
    ...
    const refMatch = expr.match(/^(b[0-9a-z]+)\s*([+\-]\s*\d+(?:\.\d+)?)?$/i);
    if (!refMatch) {
...
function readAnchor(srcRelative) {
  ...
  // Root div: the inner div immediately inside <template id="..."> — its data-composition-id
  // matches the outer wiring. Match data-anchor on ANY div within the file (first occurrence).
  const m = html.match(/data-anchor="([^"]*)"/);
...
function main() {
  ...
  if (beats.length === 0) {
    console.error("No beats parsed from index.html — check the regex / file format");
    process.exit(2);
  }
  ...
  const fails = rows.filter(
    (r) => r.status === "LATE" || r.status === "ERROR" || r.status === "NO-ANCHOR" || r.status === "ANCHOR-MISS",
  );
  const warns = rows.filter((r) => r.status === "EARLY");
  console.log(
    `\nSummary: ${rows.length} beats | ${rows.length - fails.length - warns.length} OK | ${fails.length} FAIL | ${warns.length} EARLY`,
  );

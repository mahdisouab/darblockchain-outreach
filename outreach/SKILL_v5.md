...
### LinkedIn URL search strategy (CRITICAL):
- For each lead, search for the **specific contact person's LinkedIn profile** (not just the institution page)
- Search pattern: `"[First Name] [Last Name]" "[Institution]" site:linkedin.com/in/`
- If no named contact, search for the **institution's LinkedIn page**: `"[Institution name]" site:linkedin.com/company/` or `site:linkedin.com/school/`
- Prefer `/in/` profiles (personal) over `/company/` or `/school/` pages when a named contact exists
- NEVER guess or fabricate a LinkedIn URL. Only use URLs found in search results.
...
**Vérification :** Le lien LinkedIn du décisionnaire suit le MÊME processus de vérification que les autres URLs LinkedIn (voir Step 2B). Il doit être vérifié via WebFetch avant inclusion.
